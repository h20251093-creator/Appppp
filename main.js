  ...
  if (events.length > 0) {
    return `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        <h2 style="margin: 0;">${displayTitle} 상세 일정</h2>
        <div style="display: flex; gap: 8px;">
          <button class="btn-secondary" style="padding: 6px 10px; font-size: 13px; border-radius: 8px;" onclick="window.showTimeTableModal('${dateStr}')"><i class="ph-bold ph-table"></i> 시간표</button>
          <button class="btn-primary" style="padding: 6px 10px; font-size: 13px; border-radius: 8px;" onclick="window.showAddEventModal('${dateStr}')"><i class="ph ph-plus"></i> 추가</button>
        </div>
      </div>
      ${events.map(ev => {
        let membersHtml = '';
...
window.showTimeTableModal = (targetDateStr) => {
  if (!targetDateStr) {
    const mStr = String(calDate.getMonth() + 1).padStart(2,'0');
    const yStr = calDate.getFullYear();
    const dStr = String(calDate.getDate()).padStart(2,'0');
    targetDateStr = `${yStr}-${mStr}-${dStr}`;
  }

  const dayEvents = DB.schedules.filter(s => s.date === targetDateStr);
  
  let rows = '';
  // Time slots from 10:00 to 22:00
  for(let hour = 10; hour <= 22; hour++) {
    const timeLabel = `${String(hour).padStart(2,'0')}:00`;
    
    // Find ALL events that match this hour
    const evsForHour = dayEvents.filter(e => e.time && e.time.startsWith(timeLabel.substring(0,2)));
    
    if (evsForHour.length > 0) {
      let tdContent = evsForHour.map(ev => {
        let memberStr = '';
        if (ev.members && Array.isArray(ev.members)) {
          memberStr = ev.members.map(m => m.name).join(', ');
        }
        return `
          <div style="margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px solid rgba(255,255,255,0.05);">
            <div style="font-weight:700; color:var(--text-primary); text-shadow: 0 1px 2px rgba(0,0,0,0.5);">${ev.venue}</div>
            <div style="font-size:11px; color:var(--text-secondary); margin-top:2px;">[${ev.type}] ${memberStr}</div>
          </div>
        `;
      }).join('');

      rows += `<tr style="background: rgba(157, 78, 221, 0.1);">
        <td class="time-col" style="color: var(--accent-color); vertical-align:top; border-right: 1px solid var(--stroke-color);">${timeLabel}</td>
        <td style="padding-bottom: 4px;">${tdContent}</td>
      </tr>`;
    } else {
      rows += `<tr>
        <td class="time-col" style="border-right: 1px solid var(--stroke-color);">${timeLabel}</td>
        <td style="color: rgba(255,255,255,0.2); font-style: italic;">일정 없음</td>
      </tr>`;
    }
  }
  ...
  modal.className = 'modal-backdrop';
  modal.innerHTML = `
    <div class="modal-content glass-panel" style="max-height: 80vh; display: flex; flex-direction: column;" onclick="event.stopPropagation()">
      <h3 style="margin-bottom: 4px;">타임테이블 뷰 (Table)</h3>
      <p style="color: var(--accent-secondary); font-size: 13px; font-weight: 600; margin-bottom: 16px;">${targetDateStr}</p>
      ...
      <button class="btn-secondary w-100" style="margin-top: 16px;" onclick="this.closest('.modal-backdrop').remove()">닫기</button>
    </div>
  `;
  document.getElementById('app').appendChild(modal);
...
const renderHomePage = () => {
  const p = DB.userProfile || defaultDB.userProfile;
  return `
    <div class="home-hero glass-panel" style="position: relative; overflow: hidden; padding: 24px; min-height: 180px; display: flex; flex-direction: column; justify-content: flex-end; margin-bottom: 32px; border: none;">
      <!-- Hero Background Image -->
      <div style="position: absolute; top:0; left:0; width:100%; height:100%; background-image: url('assets/vocal_practice_studio_1777189643054.png'); background-size: cover; background-position: center; opacity: 0.6;"></div>
      <div style="position: absolute; top:0; left:0; width:100%; height:100%; background: linear-gradient(180deg, transparent 0%, var(--bg-color) 100%);"></div>
      
      <div style="position: relative; z-index: 10; display: flex; justify-content: space-between; align-items: flex-end;">
        <div>
          <p style="font-size: 13px; color: var(--accent-secondary); text-transform: uppercase; letter-spacing: 2px; margin-bottom: 4px; font-weight: 700;">Welcome back</p>
          <h1 style="font-size: 32px; font-weight: 800; line-height: 1.1; letter-spacing: -0.5px; margin: 0; color: #fff;">안녕하세요,<br><span style="color: var(--accent-color);">${p.name.split(' ')[0]}</span>님.</h1>
        </div>
        <div class="user-avatar-wrapper" style="width: 56px; height: 56px; margin: 0; box-shadow: 0 0 20px rgba(0, 0, 0, 0.4);">
          <div class="user-avatar" style="width: 100%; height: 100%; border: 3px solid var(--accent-color); background: rgba(0,0,0,0.5);"><i class="ph-fill ph-user" style="font-size: 28px;"></i></div>
        </div>
      </div>
    </div>
    ...
    <div class="home-block" style="margin-bottom: 80px;">
      ...
      <div class="glass-panel feedback-card" style="padding: 0; overflow: hidden; border-radius: 20px; background: linear-gradient(145deg, rgba(20,20,25,0.7) 0%, rgba(30,30,40,0.4) 100%); border: 1px solid var(--stroke-color);">
        <div style="width: 100%; height: 120px; background-image: url('assets/bass_guitar_demo_1777189676344.png'); background-size: cover; background-position: center 30%; position: relative;">
           <div style="position: absolute; top:0; left:0; width: 100%; height: 100%; background: linear-gradient(0deg, var(--card-bg) 0%, transparent 100%);"></div>
           <i class="ph-fill ph-quotes" style="position: absolute; top: 16px; left: 16px; font-size: 40px; color: rgba(255,255,255,0.8); drop-shadow(0 2px 4px rgba(0,0,0,0.5));"></i>
        </div>
        <div style="padding: 20px; position: relative; z-index: 5;">
          <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px; margin-top: -30px;">
            <div class="user-avatar-wrapper" style="width: 38px; height: 38px; margin: 0; border: 2px solid var(--card-bg); border-radius: 50%;">
              <div class="user-avatar" style="width: 100%; height: 100%; background: var(--stroke-color);"><i class="ph-fill ph-user" style="font-size: 18px;"></i></div>
            </div>
            <div style="text-shadow: 0 1px 3px rgba(0,0,0,0.8);">
              <strong style="font-size: 14px; display: block; color: var(--text-primary);">Manager Kim</strong>
              <small style="color: var(--text-secondary); font-size: 11px;">1시간 전</small>
            </div>
          </div>
          <p style="font-size: 15px; line-height: 1.6; color: var(--text-secondary); margin-bottom: 0;">"오늘 런 너무 좋았습니다! 다음 롤링홀 공연을 대비해서 코러스 파트의 템포를 아주 조금만 낮춰보죠. 고생하셨습니다."</p>
          <button class="btn-primary w-100" style="margin-top: 20px; display: flex; justify-content: center; align-items: center; gap: 8px; font-size: 15px;" onclick="document.querySelector('.nav-item[data-target=\\'feed\\']').click()">
